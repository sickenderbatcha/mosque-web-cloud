...
const DEFAULT_FORMAT = "MAR/YYYY/NNN";
const DEFAULT_SEQUENCE = "1";

// Helper to get marriage certificate number settings
export const getMarriageCertificateNumberSettings = async () => {
  const { data } = await supabase
    .from("app_settings")
    .select("key, value")
    .in("key", ["marriage_certificate_number_format", "marriage_certificate_sequence"]);

  let format = DEFAULT_FORMAT;
  let sequence = 1;

  if (data) {
    const formatSetting = data.find(d => d.key === "marriage_certificate_number_format");
    const sequenceSetting = data.find(d => d.key === "marriage_certificate_sequence");
...
export const generateMarriageCertificateNumber = async () => {
  const { format, sequence } = await getMarriageCertificateNumberSettings();
...
export const incrementMarriageCertificateSequence = async () => {
  const { sequence } = await getMarriageCertificateNumberSettings();
  const newSequence = sequence + 1;
  
  const { data: existing } = await supabase
    .from("app_settings")
    .select("id")
    .eq("key", "marriage_certificate_sequence")
    .single();
  ...
  if (existing) {
    ...
    await supabase
      .from("app_settings")
      .insert({
        key: "marriage_certificate_sequence",
        value: newSequence.toString(),
        description: "Current sequence number for marriage certificates",
      });
  }
...
const MarriageCertificateNumberSettings = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { data: formatExisting } = await supabase
        .from("app_settings")
        .select("id")
        .eq("key", "marriage_certificate_number_format")
        .single();
      ...
      if (formatExisting) {
        ...
        await supabase
          .from("app_settings")
          .insert({
            key: "marriage_certificate_number_format",
            value: format,
            description: "Format for marriage certificate numbers (use YYYY for year, NNN for sequence)",
          });
      }
      ...
      toast({
        ...
        description: "Marriage certificate number settings have been updated.",
      });
    } catch (error: any) {
      toast({
        title: "Error",
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Hash className="h-5 w-5" />
          திருமண சான்றிதழ் எண் (Marriage Certificate Number)
        </CardTitle>
        <CardDescription>
          Configure the format and sequence for marriage certificate numbers. Use YYYY for year, NNN for 3-digit sequence.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <Label htmlFor="marriage-format">Format Pattern</Label>
            <Input
              id="marriage-format"
              value={format}
              onChange={(e) => setFormat(e.target.value)}
              placeholder="MAR/YYYY/NNN"
            />
          ...
          <div className="space-y-2">
            <Label htmlFor="marriage-sequence">Next Sequence Number</Label>
            <Input
              id="marriage-sequence"
              type="number"
              min="1"
              value={sequence}
...
export default MarriageCertificateNumberSettings;
