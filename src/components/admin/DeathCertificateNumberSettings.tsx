...
const DEFAULT_FORMAT = "DEATH/YYYY/NNN";
const DEFAULT_SEQUENCE = "1";

// Helper to get death certificate number settings
export const getDeathCertificateNumberSettings = async () => {
  const { data } = await supabase
    .from("app_settings")
    .select("key, value")
    .in("key", ["death_certificate_number_format", "death_certificate_sequence"]);

  let format = DEFAULT_FORMAT;
  let sequence = 1;

  if (data) {
    const formatSetting = data.find(d => d.key === "death_certificate_number_format");
    const sequenceSetting = data.find(d => d.key === "death_certificate_sequence");
...
export const generateDeathCertificateNumber = async () => {
  const { format, sequence } = await getDeathCertificateNumberSettings();
...
export const incrementDeathCertificateSequence = async () => {
  const { sequence } = await getDeathCertificateNumberSettings();
  const newSequence = sequence + 1;
  
  const { data: existing } = await supabase
    .from("app_settings")
    .select("id")
    .eq("key", "death_certificate_sequence")
    .single();
  ...
  if (existing) {
    ...
    await supabase
      .from("app_settings")
      .insert({
        key: "death_certificate_sequence",
        value: newSequence.toString(),
        description: "Current sequence number for death certificates",
      });
  }
...
const DeathCertificateNumberSettings = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { data: formatExisting } = await supabase
        .from("app_settings")
        .select("id")
        .eq("key", "death_certificate_number_format")
        .single();
      ...
      if (formatExisting) {
        ...
        await supabase
          .from("app_settings")
          .insert({
            key: "death_certificate_number_format",
            value: format,
            description: "Format for death certificate numbers (use YYYY for year, NNN for sequence)",
          });
      }
      ...
      toast({
        ...
        description: "Death certificate number settings have been updated.",
      });
    } catch (error: any) {
      toast({
        title: "Error",
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Hash className="h-5 w-5" />
          இறப்பு சான்றிதழ் எண் (Death Certificate Number)
        </CardTitle>
        <CardDescription>
          Configure the format and sequence for death certificate numbers. Use YYYY for year, NNN for 3-digit sequence.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <Label htmlFor="death-format">Format Pattern</Label>
            <Input
              id="death-format"
              value={format}
              onChange={(e) => setFormat(e.target.value)}
              placeholder="DEATH/YYYY/NNN"
            />
          ...
          <div className="space-y-2">
            <Label htmlFor="death-sequence">Next Sequence Number</Label>
            <Input
              id="death-sequence"
              type="number"
              min="1"
...
export default DeathCertificateNumberSettings;
