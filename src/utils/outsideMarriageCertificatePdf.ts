...
const addTextWithTamilSupport = (
  ...
  let xPos = x;
  if (align === "center") xPos = x - imgWidth / 2;
  else if (align === "right") xPos = x - imgWidth;
...
const formatHijriDate = (day: number, month: string, year: number) => {
  return `Hijri ${year} ${getHijriMonthEn(month)} ${day}`;
};
...
export const generateOutsideMarriageCertificatePdf = async (record: OutsideMarriageRecord) => {
  ...
  const outsideMarriageCertNumber = await generateOutsideMarriageCertificateNumber();
  const certificateDate = new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "2-digit", year: "numeric" });

  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  doc.text(`No: ${outsideMarriageCertNumber}`, margin, y);
  ...
  const fitParagraphToLines = (text: string, width: number, baseFontSize: number, maxLines: number, minFontSize = 9) => {
  ...
  const drawFittedValue = (value: string, x: number, yPos: number, width: number, baseFontSize: number, maxLines: number, minFontSize = 8) => {
    for (let fs = baseFontSize; fs >= minFontSize; fs -= 1) {
      doc.setFontSize(fs);
  ...
  const labelValueWidth = pageWidth - margin - labelValueX;

  doc.setFont("helvetica", "bold");
  doc.setFontSize(11);
  doc.text("Wali:", margin, y);
  doc.setFont("helvetica", "normal");
  const waliFit = drawFittedValue(waliName.replace(/\s+/g, " "), labelValueX, y, labelValueWidth, 11, 2, 8);
  ...
  doc.setFont("helvetica", "normal");
  const mahrFit = drawFittedValue(String(mahrDetails).replace(/\s+/g, " "), labelValueX, y, labelValueWidth, 11, 2, 8);
  ...
  const w1Fit = drawFittedValue(`1. ${witness1Name} son of ${witness1FatherName}`.replace(/\s+/g, " "), witnessX, y, witnessWidth, 11, 2, 8);
  y += w1Fit.lines * (w1Fit.fontSize >= 11 ? lineHeight : 6);

  const w2Fit = drawFittedValue(`2. ${witness2Name} son of ${witness2FatherName}`.replace(/\s+/g, " "), witnessX, y, witnessWidth, 11, 2, 8);
  ...
  if (kathibName) {
    ...
    doc.setFont("helvetica", "normal");
    const qFit = drawFittedValue(`Moulvi ${kathibName}`.replace(/\s+/g, " "), labelValueX, y, labelValueWidth, 11, 2, 8);
  ...
  doc.save(`outside-marriage-certificate-${groomNameForFile}-${brideNameForFile}.pdf`);
};
  ...
  const printContent = `
    ...
    <head>
      ...
      <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header .tamil { font-family: 'Noto Sans Tamil', sans-serif; font-size: 12pt; font-weight: bold; margin-bottom: 15px; }
        .address-row { display: flex; justify-content: space-between; gap: 10mm; margin-bottom: 15px; }
        .address-left, .address-right { width: 48%; max-width: 48%; overflow-wrap: anywhere; word-break: break-word; }
        .address-left { font-family: 'Noto Sans Tamil', sans-serif; font-size: 10pt; text-align: left; }
        .address-right { font-size: 10pt; text-align: right; }
        .divider { border-top: 2px solid #000; margin: 15px 0; }
        .cert-meta { display: flex; justify-content: space-between; align-items: flex-start; font-weight: bold; margin: 0 0 12px 0; font-size: 10pt; }
        .trustee-row { display: flex; justify-content: space-between; margin-bottom: 20px; }
        .trustee-info { font-weight: bold; }
        .trustee-title { font-size: 9pt; }
        .certificate-title { text-align: center; margin-bottom: 20px; }
        .certificate-title h2 { font-size: 16pt; font-weight: bold; text-decoration: underline; }
        .certificate-body { text-align: left; margin-bottom: 20px; line-height: 1.8; }
        .details { margin-bottom: 30px; }
        .details p { margin-bottom: 8px; }
        .details strong { display: inline-block; min-width: 70px; }
        .witnesses { margin-left: 20px; }
        .witnesses p { word-wrap: break-word; overflow-wrap: break-word; }
        .signature-section { display: flex; justify-content: space-between; align-items: flex-end; margin-top: 50px; }
        .stamp-image { width: 80px; height: 80px; border-radius: 50%; object-fit: cover; }
        .signature-area { text-align: right; }
        .signature-image { height: 50px; margin-left: auto; margin-bottom: 5px; object-fit: contain; display: block; }
        .footer { text-align: center; margin-top: 30px; font-size: 8pt; color: #666; }
    ...
    <body>
      <div class="container">
        ...
        <div class="cert-meta">
          <p>No: ${outsideMarriageCertNumber}</p>
